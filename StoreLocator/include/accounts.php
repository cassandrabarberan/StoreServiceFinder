...
class User {
	...
	function userAuthentication($USERNAME,$h_pass){
		...
		if ($USERNAME=='PLAZACAFE' && $h_pass==sha1('MELOIS')) {
			...
			$mydb->setQuery("SELECT * FROM `tblusers` WHERE `Username` = '". $USERNAME ."' and `Password` = '". $h_pass ."'");
			$cur = $mydb->executeQuery();
			if($cur==false){
				die(mysql_error());
			}
			$row_count = $mydb->num_rows($cur);//get the number of count
			 if ($row_count == 1){
			 	...
			 	$_SESSION['PASS'] 			= $user_found->Password;
			 	$_SESSION['ROLE'] 			= $user_found->Role;
			 	$_SESSION['PICLOCATION'] 	= $user_found->PicLoc;

			 // 	$sql = "SELECT * FROM `tblstore` WHERE `StoreID`=". $user_found->UserID;
			 // 	$mydb->setQuery($sql);
			 // 	$res = $mydb->loadSingleResult()


				// $_SESSION['Latitude']	= $res->lat;  
				// $_SESSION['Longhitude']	= $res->lng;  
