...
<table   class="table table-striped  table-hover table-responsive" style="font-size:12px" cellspacing="0">
  ...
  <tbody>
  	...
  	if($_SESSION['ADMIN_ROLE']=='Administrator'){
		$sql ="SELECT *,sum(Stocks) as 'sto',sum(Sold) as sol,sum(`ExpiredQty`) as exp,sum(Remaining) as rem  FROM tblstockin st, `tblproduct` p, `tblcategory` c,`tblstore` s WHERE st.ProductID=p.ProductID AND p.`CategoryID`=c.`CategoryID` AND p.`StoreID`=s.`StoreID` GROUP BY p.ProductID";
  	}else{
